...
function sendEmailBrevo(data) {
  ...
  if (!apiKey) {
    // FIX: throw au lieu de return — sinon doPost croit que l'email est parti
    throw new Error('BREVO_API_KEY manquante — ajouter dans Paramètres du projet → Propriétés de script');
  }
  ...
  if (code !== 201) {
    console.error(`Brevo erreur ${code} : ${body}`);
    throw new Error(`Brevo ${code} — ${body}`);
  }
